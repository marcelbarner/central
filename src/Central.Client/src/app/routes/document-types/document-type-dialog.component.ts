...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>
      {{ (data ? 'documentTypes.edit' : 'documentTypes.create') | translate }}
    </h2>

    <form [formGroup]="form" (ngSubmit)="onSubmit()">
      <mat-dialog-content class="space-y-4">
        <mat-form-field class="w-full">
          <mat-label>{{ 'documentTypes.name' | translate }}</mat-label>
        ...
        <mat-form-field class="w-full">
          <mat-label>{{ 'documentTypes.description' | translate }}</mat-label>
          <textarea
            matInput
...
export class DocumentTypeDialogComponent {
  private readonly dialogRef = inject(MatDialogRef<DocumentTypeDialogComponent>);
  private readonly fb = inject(FormBuilder);
  readonly data = inject<DocumentType | undefined>(MAT_DIALOG_DATA);
